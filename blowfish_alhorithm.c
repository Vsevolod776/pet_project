...
static const uint32_t SArray[4][256] = {
        { /* S1 */
...
void encrypt(uint32_t *L, uint32_t *R, const BlowfishKey *key)
{
    ...
    for (int i = 0; i < 16; i++)
    {
        //XOR с текущим значением P-блока
        Xl = Xl ^ key->P[i];
        //Извлечение байтов
        uint32_t a = (Xl >> 24) & 0xFF;
        uint32_t b = (Xl >> 16) & 0xFF;
        uint32_t c = (Xl >> 8) & 0xFF;
        uint32_t d = Xl & 0xFF;
        //Подстановка в S-блок
        uint32_t S1 = key->S[0][a];
        uint32_t S2 = key->S[1][b];
        uint32_t S3 = key->S[2][c];
        uint32_t S4 = key->S[3][d];
        // Сложение результатов первых двух подстановок
        uint32_t combined1 = S1 + S2;
        // XOR с результатом третьей подстановки
        uint32_t combined2 = combined1 ^ S3;
        // Сложение с результатом четвертой подстановки
        uint32_t combined3 = combined2 + S4;
        // XOR с текущим значением `Xr`
        Xr = combined3 ^ Xr;
        uint32_t buffer = Xl;
        Xl = Xr;
        Xr = buffer;
...
{
    memcpy(key->P, PArray, sizeof(PArray));
    memcpy(key->S, SArray, sizeof(SArray));
    ...
    for (int i = 0; i < 18; ++i)
    {
        ...
        for (int k = 0; k < 4; ++k) {
            data = (data << 8) | key_data[j];
            j = j + 1;
            if (j >= key_length) {
                j = 0;
            }
        }
        key->P[i] = key->P[i] ^ data;
    }
